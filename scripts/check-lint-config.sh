...
#
# Reads scripts/protected-files.yaml to determine WHAT to protect.
...
#
# Usage:
#   scripts/check-lint-config.sh            # verify (CI)
#   scripts/check-lint-config.sh --update    # append new entry (needs password)
#   scripts/check-lint-config.sh --history   # show the chain log
#   scripts/check-lint-config.sh --show      # show what's being protected
set -euo pipefail
...
if [ ! -f "$CHAIN_FILE" ]; then
  ...
  echo "FAIL: Missing $CHAIN_FILE — run: scripts/check-lint-config.sh --update" >&2
  exit 1
fi
...
if [ "$entry_count" -eq 0 ]; then
  ...
  echo "FAIL: $CHAIN_FILE is empty — run: scripts/check-lint-config.sh --update" >&2
  exit 1
fi
...
if [ "$current_ch" != "$last_ch" ]; then
  ...
  echo "    scripts/check-lint-config.sh --update" >&2
  echo "" >&2
